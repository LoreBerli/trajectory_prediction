...
def get_sample_batch(data_dir):
    ...
    with open(path.join(data_dir,'filtered_data.txt'), 'r') as f:
        logs = f.readlines()
        ...
        for log in logs[0:10]:
            ...
            # load images
            frame_frontal_path = path.join(data_dir+"/frames", frame_f.strip())
            frame_birdeye_path = path.join(data_dir+"/frames", frame_b.strip())
...
print(X_coords[0])
plt.imsave("prova.png",Y_crops[0])
